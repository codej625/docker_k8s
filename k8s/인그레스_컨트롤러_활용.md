...
# 먼저 현재 taint 확인
kubectl describe node k3s-master | grep Taints

# taint 적용
kubectl taint nodes k3s-master node-role.kubernetes.io/control-plane:NoSchedule --overwrite
```
...
```zsh
# 환경변수 설정 후 설치
export K3S_URL="https://192.168.0.10:6443" # IP 그대로 복붙 하면 안 됨
export K3S_TOKEN="<VM1에서_확인한_토큰값>"
...
```
# 백엔드에서 DB 연결 시 서비스 이름 사용
Spring Boot application.properties 예시:

spring.datasource.url=jdbc:postgresql://database-service:5432/dbname
spring.datasource.username=postgres
spring.datasource.password=yourpassword

쿠버네티스 내부 DNS가 자동으로 서비스 이름을 ClusterIP로 해석한다.
```
